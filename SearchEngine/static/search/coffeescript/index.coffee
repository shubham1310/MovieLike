...
print_results = (result) ->
  ...
  for doc in result.results
    ...
    html = "<li style = 'height:200px;margin-bottom:50px;'> <div style='margin-top:-10px'> <a href='http://www.imdb.com/title/#{doc.path}' style='text-decoration : none' target='_blank'> <div style = 'height:170px;float:left;margin-right: 50px; display: inline-block;'>  <h4 style='margin-top:-0.5px'>#{doc.name}</h4> <p style='font-size:16px;max-width: 400px;'> <img src='./image/star.png' alt='Rating' style='width:30px;height:30px;'> #{doc.rating} &nbsp;| #{doc.time} <em style='float:right'> #{doc.genre}</em></br>#{doc.title} </br> <small>#{doc.score.toFixed(4)}</small></p> </div> <div style = 'height:170px;float:right;margin-right: 50px; display: inline-block;'> <img src='./image/#{doc.namestrip}.jpg' alt='Rating' style='width:150px;height:180px;'> </div> </div> </a> </li>"
